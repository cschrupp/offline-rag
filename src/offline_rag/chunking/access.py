"""Read-only CURRENT / historical ChunkSet access (neutral infrastructure)."""
    ...
    *,
    ...
    if corpus_manifest_name is not None:
        ...
        if not corpus_manifest_path.exists():
            raise ChunkAccessError(f"corpus manifest missing: {corpus_manifest_name}")
        try:
    ...
    for entry in chunk_manifest.documents:
        ...
        if not artifact_path.exists():
            raise ChunkAccessError(f"chunk artifact missing: {entry.chunk_artifact_id}")
