...
**Phase:** Milestone 4 in progress — Slices 0–9, **9A**, **9B**, and **9C** done; next decision track is **Slice 9D** (relevance prelabel).

Working local path: ingest → chunk → index / index lexical → retrieve ladder → `query` → `eval retrieve` / `eval compare` → `eval query` → `doctor` → `gold propose` → `gold pool`.

Still deferred: remaining authoring slices (9D–9H), semantic answer/citation quality metrics (Milestone 5 / Slice 10), agentic recovery (Milestone 6).
