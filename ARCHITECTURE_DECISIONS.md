...
**Consequence:** Implementation proceeds strictly 9A→9H. Slice **9A** (contracts + privacy boundary) is complete; question generation starts at **9B**. DeepEval/Ragas/Giskard/NotebookLM are not required architecture. Promotion of Arm H / query-prompt / provenance-v2 remains an explicit later decision (9H for retrieval contracts; Slice 10 for provenance-v2). Canonical plan: `docs/milestone4_offline_gold_authoring.md`. Slice 9A notes: `docs/slice9a_gold_authoring.md`.
